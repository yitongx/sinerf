...
def parse_args():
    ...
    parser.add_argument('--base_dir', type=str, default='./data_dir/LLFF')
...
def gen_detail_name(args):
    outstr = 'lr_' + str(args.nerf_lr) + \
             ...
             '_' + str(args.alias) + \
             '_' + str(datetime.datetime.now().strftime('%m%d_%H%M'))
    return outstr
...
def main(args):
    ...
    '''Create Folders'''
    exp_root_dir = Path(os.path.join('./outputs', args.scene_name))
