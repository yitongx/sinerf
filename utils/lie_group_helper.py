...
def get_G(r):
    ...
    Args:
        r (_type_): (3, ) axis angle
    """
    skew_r = vec2skew(r)
    norm_r = r.norm() + 1e-15
    eye3x3 = torch.eye(3, dtype=torch.float32, device=r.device)
    G = eye3x3 + ((1 - torch.cos(norm_r)) / (norm_r**2)) * skew_r + ((norm_r - torch.sin(norm_r))/(norm_r**3)) * (skew_r @ skew_r)
    return G
        
def make_c2w_screw_motion(r, v):
    """
    Following Nerfies
    t = Gv
